...
export default function TicketWinClaim({ win }: { win: TicketWin }) {
    ...
    <div style={cardStyle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
        {win.image
          ...
          ? <img src={win.image} alt="" style={{ width: '46px', height: '46px', objectFit: 'cover', borderRadius: '8px', flexShrink: 0 }} />
          : <span style={{ fontSize: '1.9rem', lineHeight: 1 }}>🎁</span>}
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: '0.62rem', fontWeight: 800, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#c2a24e', marginBottom: '0.2rem' }}>Instant Win prize</p>
          <p style={{ fontFamily: 'var(--font-cormorant, serif)', fontSize: '1.2rem', fontWeight: 600, color: '#fff', lineHeight: 1.2 }}>{win.name || 'Prize'}</p>
          <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.6)', marginTop: '0.15rem' }}>
            Ticket Nº {String(win.ticketNo).padStart(4, '0')}{win.amount > 0 ? ` · worth £${win.amount.toLocaleString()}` : ''}
          </p>
        </div>
        {claimed
